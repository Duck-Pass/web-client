...
import { UserKey, MasterKey, SymmetricCryptoKey } from "../models/symmetric-crypto-key";
...
export class CryptoService implements CryptoServiceAbstract {
    ...
    async makeMasterKey(password: string, email: string) : Promise<MasterKey> {
        ...
        const key = await this.primitiveService.pbkdf2(password, email, iterations);
    ...
    async hashMasterKey(
        ...
        const hash = await this.primitiveService.pbkdf2(key.key, password, iterations);
        return BufferUtils.fromBufferToBase64(hash);
    }

    async makeUserKey(masterKey: MasterKey) : Promise<[UserKey, EncryptedString]> {
    ...
    private async makeProtectedSymmetricKey<T extends SymmetricCryptoKey>(
        ...
        if (encKey.key.byteLength === 32) {
            ...
            psk = await this.encryptionService.encrypt(newSymkey, stretchedEncryptionKey);
    ...
    private async stretchMasterKey(key: SymmetricCryptoKey) : Promise<SymmetricCryptoKey> {
        const newKey = new Uint8Array(64);
        const encKey = await this.primitiveService.hkdfExpand(key.key, "enc", 32)
        const macKey = await this.primitiveService.hkdfExpand(key.key, "mac", 32)
        newKey.set(encKey);
